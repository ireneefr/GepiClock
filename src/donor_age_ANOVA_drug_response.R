# in this script is computed the same ANOVA analysis but with age_at_sampling variable instead of age_prediction to make a comparison
...
GDSC_age <- generate_merged_drug_dataset(GDSC_combined, cl_samples_with_age_at_sampling, threshold = 10) # load data with annotated donor age
head(GDSC_age)
print(paste("In the pan-cancer analysis, the number of cell lines (with annotated donor age)
 after the category selection is", length(unique(GDSC_age$COSMIC_ID))))
...
# considering tissue/cancer type and MSI status.A correlation analysis is performed between residuals and age at sampling to assess whether age 
...
anova_drug_pancancer_results <- anova_analysis_drugresponse_pan(GDSC_age, "age_at_sampling")
# plot correlation between age at sampling and residual drug sensitivity
plot_predage_LNIC50_correlation_pan_drug(anova_drug_pancancer_results$correlation_results,
output_dir = paste0(figures_path, "donor_age_volcano_ANOVA_drug_pan_cancer.png"))
...
# between age at sampling and residual drug sensitivity (LN_IC50).
...
output_dir = "results/cell_lines/cell_lines_drug_sensitivity/dsea_pancancer/donor_age_significant_enrichment_dsea_pancancer.xlsx")
...
output_dir = paste0(figures_path, "donor_age_DSEA_pancancer_dotplot.png"))
plot_target_drug_distribution(dsea_pan_cancer_results, ordered_targets, 
output_dir = paste0(figures_path, "donor_age_stacked_bar_plot_pathway_drugs_ordered.png"))
...
print(paste("In the cancer type-specific analysis, the number of cell lines (with annotated donor age)
 after the category selection is", length(unique(GDSC_age_filtered$COSMIC_ID))))
...
# and assesses correlation with age at sampling.
# the aim is the identification of cancer types where drug response is significantly age-associated.

# perform anova anova model for drug response
cancer_specific_anova_results <- anova_drugresponse_cancer_specific(GDSC_age_filtered, "age_at_sampling")
summarize_significant_drugs(cancer_specific_anova_results, 
output_file = "results/cell_lines/cell_lines_drug_sensitivity/significant_drugs_cancer_types/donor_age_significant_drugs_anova_results.xlsx")
