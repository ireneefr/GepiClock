...
for(clock in seq_along(clock_names)){
  ...
  for(i in 1:20){ # 20-CV
    ...
    pred_test <- AgePred_tcga(name = paste0("missing/test_", clock_names[clock], "_", i, ".txt"),
                              coefs = coefs,
                              bval = t(x.test[, colnames(x.test) %in% keep_coefs]))
    pred_normal <- AgePred_tcga(name = paste0("missing/normal_", clock_names[clock], "_", i, ".txt"),
                                coefs = coefs,
                                bval = t(x.normal[, colnames(x.normal) %in% keep_coefs]))
    pred_tumor <- AgePred_tcga(name = paste0("missing/tumor_", clock_names[clock], "_", i, ".txt"),
                               coefs = coefs,
    ...
    row_test <- c(clock_names[clock], clock_cpgs[clock], i, r_test, mae_test)
    row_normal <- c(clock_names[clock], clock_cpgs[clock], i, r_normal, mae_normal)
    row_tumor <- c(clock_names[clock], clock_cpgs[clock], i, r_tumor, mae_tumor)
    res_test <- rbind(res_test, row_test)
    res_normal <- rbind(res_normal, row_normal)
    res_tumor <- rbind(res_tumor, row_tumor)
  }
}
colnames(res_test) <- c("Clock_name", "Clock_cpgs", "Iteration", "R", "MAE")
colnames(res_normal) <- c("Clock_name", "Clock_cpgs", "Iteration", "R", "MAE")
colnames(res_tumor) <- c("Clock_name", "Clock_cpgs", "Iteration", "R", "MAE")
write.csv(res_test, paste0(dir_results, "model/test_clock_cpgs.csv"))
write.csv(res_normal, paste0(dir_results, "model/normal_clock_cpgs.csv"))
write.csv(res_tumor, paste0(dir_results, "model/tumor_clock_cpgs.csv"))
